...
public class JdMantCurso extends JDialog implements ActionListener, ItemListener, MouseListener {
	...
	protected void itemStateChangedCboCodigo(ItemEvent e) {
		try {
			...
			txtCreditos.setText("" + buscado.getNumeroCredito());
			txtHoras.setText("" + buscado.getCamtidadHora());
					...
					try {
						...
						try {
							...
							if (!btnAdicionar.isEnabled()) {
								...
								buscado.setNumeroCredito(creditos);
								buscado.setCamtidadHora(horas);
	...
	void listar() {
		...
		for (int i = 0; i < ac.tamaño(); i++) {
			Object[] fila = {
					...
					ac.obtener(i).getNumeroCredito(),
					ac.obtener(i).getCamtidadHora()
			};
			modelo.addRow(fila);
		}
	}
