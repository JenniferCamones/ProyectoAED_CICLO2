...
public class JdRegRetiro extends JDialog implements ItemListener, ActionListener, MouseListener {
	...
	protected void itemStateChangedCboNumRetiro(ItemEvent e) {
		try {
			...
			Matricula buscadoM = am.buscar(buscadoR.getNumeroMatricula());
			cboNumMatricula.setSelectedItem(buscadoR.getNumeroMatricula());
			...
			try {
				...
				if (!btnAdicionar.isEnabled()) {
					if (aa.buscar(am.buscar(numMatricula).getCodigoAlumno()).getEstado() == 1) {
						...
						am.buscar(buscado.getNumeroMatricula()).setCodigoCurso(codCurso);
	...
	protected void actionPerformedBtnEliminar(ActionEvent e) {
		try {
			...
			if (confirmar() == 0) {
				ar.eliminar(buscado);
				aa.buscar(am.buscar(buscado.getNumeroMatricula()).getCodigoAlumno()).setEstado(1);
				aa.ActualizarArchivo();
				listar();
	...
	void listarCboNumRetiro() {
		...
		for (int i = 0; i < ar.tamaño(); i++) {
			cboNumRetiro.addItem(ar.obtener(i).getNumeroRetiro());
		}
	}
	...
	void listarCboCodCurso() {
		...
		for (int i = 0; i < ac.tamaño(); i++) {
			cboCodigoCurso.addItem(ac.obtener(i).getCodigo());
		}
	}
	...
	void listar() {
		...
		for (int i = 0; i < ar.tamaño(); i++) {
			Object[] fila = {
					ar.obtener(i).getNumeroRetiro(),
					ar.obtener(i).getNumeroMatricula(),
					aa.buscar(am.buscar(ar.obtener(i).getNumeroMatricula()).getCodigoAlumno()).getnombres(),
					aa.buscar(am.buscar(ar.obtener(i).getNumeroMatricula()).getCodigoAlumno()).getApellidos(),
					ac.buscar(am.buscar(ar.obtener(i).getNumeroMatricula()).getCodigoCurso()).getAsignatura(),
	...
	protected void mouseClickedTblRetiro(MouseEvent e) {
		if (btnAdicionar.isEnabled()) {
			try {
				...
				cboNumMatricula.setSelectedItem(buscado.getNumeroMatricula());
			} catch (Exception error) {
